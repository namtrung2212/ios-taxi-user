...
open class NotYetPaidTable : UITableViewController, DTTableViewManageable {
    
    open var parent: NotYetPaidScreen
    
    public init(parent: NotYetPaidScreen){
        ...
        self.refreshControl!.addTarget(self, action: #selector(NotYetPaidTable.refresh(_:)), for: .valueChanged)
        self.tableView.addSubview(self.refreshControl!)
        
        manager.startManagingWithDelegate(self)
        manager.registerCellClass(TravelOrderCell)
        manager.cellSelection(NotYetPaidTable.selectedItem)
    ...
    open func invalidate( _ completion: (() -> ())?){
        ...
        TravelOrderController.GetNotYetPaidOrderByUser(SCONNECTING.UserManager!.CurrentUser!.id!) { (orders) in
