...
open class NotYetPaidScreen: UIViewController {
    
    var historyTable: NotYetPaidTable!
    ...
    func initControls(_ completion: (() -> ())?){
        ...
        let btnBack = UIBarButtonItem(title: "", style: .plain, target: self, action: #selector(NotYetPaidScreen.btnBack_Clicked))
        btnBack.setFAIcon(FAType.faArrowLeft, iconSize : 20)
        btnBack.setTitlePositionAdjustment(UIOffsetMake(0, -5), for: .default)
        self.navigationItem.leftBarButtonItem = btnBack
        self.navigationItem.title = "Chưa thanh toán"
