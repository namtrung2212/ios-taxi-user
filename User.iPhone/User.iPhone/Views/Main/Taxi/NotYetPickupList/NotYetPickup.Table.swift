...
open class NotYetPickupTable : UITableViewController, DTTableViewManageable {
    
    open var parent: NotYetPickupScreen
    
    public init(parent: NotYetPickupScreen){
        ...
        self.refreshControl!.addTarget(self, action: #selector(NotYetPickupTable.refresh(_:)), for: .valueChanged)
        self.tableView.addSubview(self.refreshControl!)
        
        manager.startManagingWithDelegate(self)
        manager.registerCellClass(TravelOrderCell)
        manager.cellSelection(NotYetPickupTable.selectedItem)
    ...
    open func invalidate(){
        ...
        TravelOrderController.GetNotYetPickupOrderByUser(SCONNECTING.UserManager!.CurrentUser!.id!) { (orders) in
