...
open class NotYetPickupScreen: UIViewController {
    
    var tableList: NotYetPickupTable!
    ...
    func initControls(_ completion: (() -> ())?){
        ...
        let btnBack = UIBarButtonItem(title: "", style: .plain, target: self, action: #selector(NotYetPickupScreen.btnBack_Clicked))
        btnBack.setFAIcon(FAType.faArrowLeft, iconSize : 20)
        btnBack.setTitlePositionAdjustment(UIOffsetMake(0, -5), for: .default)
        self.navigationItem.leftBarButtonItem = btnBack
        self.navigationItem.title = "Chưa khởi hành"
        ...
        tableList = NotYetPickupTable(parent: self)
